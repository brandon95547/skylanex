...
export const home = {
  ...
  <section class="border-y border-surface-800 px-5 py-8 sm:px-8">
    <div class="mx-auto max-w-5xl">
      <p class="mb-5 text-center text-xs font-semibold uppercase tracking-widest text-fg-muted">Trusted by teams &amp; products including</p>
      <div class="flex flex-wrap items-center justify-center gap-x-10 gap-y-4">
        ${trustedBy.map((n) => `<span class="text-lg font-bold text-fg-secondary">${n}</span>`).join("")}
      </div>
    </div>
  </section>
        ...
        <div class="relative grid gap-8 lg:grid-cols-[1.3fr_1fr] lg:items-center">
          ...
          <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
            ${[
              ...
              <span class="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-primary-500/10 text-primary-200 ring-1 ring-inset ring-primary-500/20">${icon(p.icon, "h-4 w-4")}</span>
              <span class="min-w-0">
                <span class="block truncate text-sm font-semibold text-surface-50">${p.name}</span>
                <span class="block truncate text-xs text-fg-muted">${p.sub}</span>
              </span>
            </div>`
              )
              .join("")}
  ...
  <section class="border-y border-surface-800 bg-surface-950 px-5 py-12 sm:px-8">
    <div class="mx-auto grid max-w-5xl gap-8 sm:grid-cols-2 lg:grid-cols-4">
      ${stats
        ...
        <span class="grid h-12 w-12 shrink-0 place-items-center rounded-xl bg-primary-500/10 text-primary-200 ring-1 ring-inset ring-primary-500/20">${icon(["calendar", "rocket", "users", "code"][i] || "spark", "h-5 w-5")}</span>
        <span>
          <span class="block text-3xl font-extrabold tracking-tight text-fg">${st.value}</span>
          <span class="block text-sm text-fg-muted">${st.label}</span>
        </span>
      </div>`
        )
        .join("")}
      ...
      <div class="relative mt-14">
        ...
        <ol class="relative grid gap-10 md:grid-cols-4 md:gap-6">
          ${[
            ...
            <h3 class="mt-4 text-base font-semibold text-fg">${step.t}</h3>
            <p class="mx-auto mt-2 max-w-[15rem] text-sm leading-relaxed text-fg-secondary">${step.d}</p>
          </li>`
            )
            .join("")}
        </ol>
