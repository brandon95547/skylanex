...
function concept(c, industrySlug) {
  ...
  return `<figure class="reveal group">
    ...
    <figcaption class="mt-3 flex flex-wrap items-baseline justify-between gap-2">
      <span class="text-sm font-semibold text-fg">${c.label}</span>
      <span class="text-xs text-fg-secondary">${c.firm}</span>
    </figcaption>
    <a href="${href}" class="mt-2 inline-flex items-center gap-1.5 text-sm font-semibold text-primary-200 hover:text-primary-200">
...
function conceptDetail(p, c) {
  ...
  ${heroGlow(`
    <div class="mx-auto max-w-5xl px-5 pb-10 pt-20 sm:px-8 sm:pt-24">
      <a href="/solutions/${p.slug}#concepts" class="inline-flex items-center gap-1.5 text-sm text-fg-muted hover:text-primary-200">${icon("arrow", "h-4 w-4 rotate-180")} All ${p.eyebrow} concepts</a>
      <p class="eyebrow mt-6 text-secondary-400">${c.label}</p>
      <h1 class="mt-2 max-w-3xl text-4xl font-extrabold leading-tight tracking-tight text-fg sm:text-5xl">${c.firm}</h1>
      <p class="mt-5 max-w-2xl text-lg leading-relaxed text-fg-secondary">${c.blurb}</p>
  ...
  <section class="px-5 py-14 sm:px-8">
    <div class="mx-auto grid max-w-6xl gap-12 lg:grid-cols-[1.4fr_1fr]">
      <div>
        <h2 class="text-2xl font-bold text-fg sm:text-3xl">What this concept does</h2>
        <ul class="mt-8 space-y-5">
          ${c.points
            .map(
              (pt) => `<li class="flex items-start gap-3">
            <span class="mt-0.5 shrink-0 text-primary-200">${icon("check", "h-5 w-5")}</span>
            <p class="leading-relaxed text-fg-secondary">${pt}</p>
          </li>`
            )
            .join("")}
        </ul>
      ...
      ${
        ...
        <h3 class="text-sm font-semibold text-fg">Color direction</h3>
        <p class="mt-1 text-xs text-fg-secondary">${palette.name}</p>
        <div class="mt-5 flex gap-2">
          ${swatchKeys
            ...
            <p class="mt-1.5 truncate text-[11px] text-fg-secondary">${label}</p>
          </div>`
            )
            .join("")}
        </div>
        <p class="mt-5 text-xs leading-relaxed text-fg-secondary">A starting direction. Your palette is chosen with you — this is where the conversation begins, not where it ends.</p>
      </aside>`
          : ""
      }
    </div>
...
function detail(p) {
  ...
  ${heroGlow(`
    <div class="mx-auto max-w-5xl px-5 pb-12 pt-20 sm:px-8 sm:pt-24">
      <a href="/solutions" class="inline-flex items-center gap-1.5 text-sm text-fg-muted hover:text-primary-200">${icon("arrow", "h-4 w-4 rotate-180")} All solutions</a>
      <div class="mt-6 flex items-start gap-5">
        <span class="hidden h-14 w-14 shrink-0 place-items-center rounded-2xl bg-primary-500/10 text-primary-200 sm:grid">${icon(p.icon, "h-7 w-7")}</span>
        <div>
          <p class="eyebrow text-secondary-400">${p.eyebrow}</p>
          <h1 class="mt-2 max-w-3xl text-4xl font-extrabold leading-tight tracking-tight text-fg sm:text-5xl">${p.h1}</h1>
          <p class="mt-5 max-w-2xl text-lg leading-relaxed text-fg-secondary">${p.lede}</p>
        </div>
      </div>
      <div class="mt-8 flex flex-wrap gap-3">
      ...
      <div class="mt-12 grid max-w-2xl grid-cols-3 gap-6 border-t border-surface-800 pt-8">
        ${p.heroStats
          .map(
            (s) => `<div>
          <p class="text-3xl font-extrabold tracking-tight text-fg">${s.v}</p>
          <p class="mt-1 text-xs text-fg-muted">${s.l}</p>
        </div>`
          )
          .join("")}
      </div>
  ...
  <section id="concepts" class="scroll-mt-24 px-5 py-16 sm:px-8">
    <div class="mx-auto max-w-6xl">
      ...
      <p class="mt-8 max-w-3xl text-sm leading-relaxed text-fg-muted">
  ...
  <section class="border-y border-surface-800 px-5 py-16 sm:px-8">
    <div class="mx-auto max-w-5xl text-center">
      <p class="eyebrow text-primary-200">Practice areas</p>
      <h2 class="mt-3 text-2xl font-bold text-fg sm:text-3xl">Every practice area gets its own page</h2>
      <p class="mx-auto mt-4 max-w-2xl text-fg-secondary">Nobody searches for "law firm." They search for the thing that happened to them. Each of these becomes a real page, written and structured to be found.</p>
      <div class="mt-8 flex flex-wrap justify-center gap-2">
        ${p.segments
          .map((s) => `<span class="rounded-full border border-surface-700 px-3.5 py-1.5 text-sm text-fg-secondary">${s}</span>`)
          .join("")}
      </div>
    </div>
  </section>
    ...
    <div class="mx-auto max-w-6xl">
      ...
      <div class="mt-10 grid gap-5 sm:grid-cols-2">
        ${p.differentiators
          ...
          <span class="mb-4 inline-grid h-10 w-10 place-items-center rounded-lg bg-primary-500/10 text-primary-200">${icon(d.icon, "h-5 w-5")}</span>
          <h3 class="text-base font-semibold text-fg">${d.t}</h3>
          <p class="mt-2 text-sm leading-relaxed text-fg-muted">${d.d}</p>
        </div>`
          )
          .join("")}
      </div>
  ...
  <section class="px-5 pb-20 sm:px-8">
    <div class="mx-auto grid max-w-6xl gap-12 lg:grid-cols-[1.25fr_1fr]">
      <div>
        <h2 class="text-2xl font-bold text-fg sm:text-3xl">What ships</h2>
        <div class="mt-8 grid gap-4 sm:grid-cols-2">
          ${p.included
            .map(
              (i) => `<div class="rounded-2xl border border-surface-800 bg-surface-900/40 p-5">
            <div class="flex items-start gap-2.5">
              <span class="mt-0.5 shrink-0 text-primary-200">${icon("check", "h-4 w-4")}</span>
              <div>
                <h3 class="text-sm font-semibold text-fg">${i.t}</h3>
                <p class="mt-1.5 text-sm leading-relaxed text-fg-muted">${i.d}</p>
              </div>
            </div>
          </div>`
            )
      ...
      <aside>
        <h2 class="text-2xl font-bold text-fg sm:text-3xl">Found, not just built</h2>
        <ul class="mt-8 space-y-4">
          ${p.seoPoints
            .map(
              (s) => `<li class="rounded-xl border border-surface-800 bg-surface-950/60 p-4">
            <h3 class="text-sm font-semibold text-fg">${s.t}</h3>
            <p class="mt-1.5 text-sm leading-relaxed text-fg-muted">${s.d}</p>
          </li>`
            )
            .join("")}
        </ul>
      ...
      <div class="mt-10 divide-y divide-surface-800 border-y border-surface-800">
        ${p.faqs
          ...
          <summary class="flex cursor-pointer list-none items-center justify-between gap-4 py-5 text-left text-base font-semibold text-surface-100 marker:hidden">
            <span>${f.q}</span>
            <span class="shrink-0 text-primary-200 transition-transform group-open:rotate-45">${icon("spark", "h-5 w-5")}</span>
          </summary>
          <p class="pb-5 -mt-1 max-w-2xl text-sm leading-relaxed text-fg-muted">${f.a}</p>
        </details>`
          )
          .join("")}
      </div>
